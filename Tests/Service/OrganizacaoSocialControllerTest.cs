...
namespace Tests.Service
{
    public class OrganizacaoSocialControllerTest : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        [Theory]
        [InlineData("api/organizacao-social")]
        public async Task GetByCnpj_OrganizacaoesSociais_PosInsert_Valido(string baseUrl)
        {
            // Arrange
            var client = _factory.CreateClient();
            var url = "api/organizacao-social";
            ...
            var OrganizacaoSocialcommand = new OrganizacaoSocialCommand
            {
                Identificacao = new OrganizacaoSocialIdentificacaoCommand
                {
                    ...
                    Telefone = "1234567890",
                    Cnpj = novoCnpj,
                    ArtigoReferencia = "Artigo teste GetBy Id",
                    Finalidade = 1, // Exemplo de finalidade
                    FinalidadeResumida = "Uma organização social de exemplo."
                },
                Endereco = new EnderecoCommand
        ...
        [Theory]
        [InlineData("api/organizacao-social")]
        public async Task GetById_OrganizacaoesSociais_Valido(string baseUrl)
        {
            // Arrange
            var client = _factory.CreateClient();
            var url = "api/organizacao-social";
            ...
            client.DefaultRequestHeaders.Add("X-Tenant", "bora");
            var urlGetbyId = $"{baseUrl}/{id}";
            var responseGet = await client.GetAsync(urlGetbyId);
            // Assert
            Assert.True(responseGet.EnsureSuccessStatusCode().IsSuccessStatusCode);
        ...
        */
        ...
        [Theory]
        [InlineData("api/organizacao-social")]
        public async Task GetById_OrganizacaoesSociais_ValidateCnpj_ReturnsExpectedCnpj(string baseUrl)
        {
            // Arrange
            var client = _factory.CreateClient();
            var url = "api/organizacao-social";
            ...
            // Verificar se a organização foi salva corretamente
            var getByCnpjUrl = $"{url}/cnpj/{novoCnpj}";
            var getResponseAux = await client.GetAsync(getByCnpjUrl);
            getResponseAux.EnsureSuccessStatusCode();

            // Pegar Id
            var responseBody = await getResponseAux.Content.ReadAsStringAsync();
            var json = JObject.Parse(responseBody);
            var id = json["id"]?.ToString();
            ...
            // Act
            _client.DefaultRequestHeaders.Add("X-Tenant", "bora");
            var responseGetById = await _client.GetAsync(urlGetbyId);

            // Assert
            responseGetById.EnsureSuccessStatusCode();
            
            var responseBodyGetById = await responseGetById.Content.ReadAsStringAsync();
            var jsonGetId = JObject.Parse(responseBody);
            var cnpj = jsonGetId["cnpj"]?.ToString();
        ...
        [Theory]
        [InlineData("api/organizacao-social")]
        public async Task AlterarStatus_OrganizacaoSocial_Valido(string baseUrl)
        {
